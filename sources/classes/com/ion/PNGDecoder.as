		...
		*******************************/
		...
		public function decode(ba:ByteArray):BitmapData {
			...
			for (var i:int = 0; i < chunks.length; ++i) {
				...
				if (i == 0) {
					if (chunks[i].type == "IHDR") validChunk = processIHDR(chunks[i].length);
				}
				else {
					if (chunks[i].type == "PLTE") validChunk = processPLTE(chunks[i].length);
					else
					if (chunks[i].type == "IDAT") buffer.writeBytes(fileIn, fileIn.position, chunks[i].length);
				}
 
				if (chunks[i].type == "IHDR") ++countIHDR;
				if (chunks[i].type == "PLTE") ++countPLTE;
 
				if (!validChunk || countIHDR > 1 || countPLTE > 1) return null;
			}
		...
		private function readSignature():Boolean {
			if (fileIn.readUnsignedInt() != 0x89504e47) return false;
			if (fileIn.readUnsignedInt() != 0x0D0A1A0A) return false;
		...
		private function processIHDR(cLength:int):Boolean {
			...
			if (imageInfo[0] <= 0 || imageInfo[1] <= 0) return false;
			...
			switch (imageInfo[3]) {
				case 0: if (imageInfo[2] != 1 && imageInfo[2] != 2 && imageInfo[2] != 4 && imageInfo[2] != 8 && imageInfo[2] != 16) return false; break;
				case 2: if (imageInfo[2] != 8 && imageInfo[2] != 16) return false; break;
				case 3: if (imageInfo[2] != 1 && imageInfo[2] != 2 && imageInfo[2] != 4 && imageInfo[2] != 8) return false; break;
				case 4:	if (imageInfo[2] != 8 && imageInfo[2] != 16) return false; break;
				case 6: if (imageInfo[2] != 8 && imageInfo[2] != 16) return false; break;
				default: return false;
			}
 
			if (imageInfo[4] != 0 || imageInfo[5] != 0) return false;
			if (imageInfo[6] != 0 && imageInfo[6] != 1) return false;
 
			return true;
		}
 
		private function processPLTE(cLength:int):Boolean {
			if (cLength % 3 != 0) return false;
			if (imageInfo[3] == 0 || imageInfo[3] == 4) return false;
 
			trace("falta processPLTE");
		...
		private function processIDAT():BitmapData {
			...
			while (buffer.position < buffer.length) {
				...
				//trace(scanline, filter)
 
				for (var i:int = 0; i < imageInfo[0]; ++i) {
					switch (filter) {
						case 0: bd.setPixel32(i, scanline, noFilter()); break;
						case 1: bd.setPixel32(i, scanline, subFilter()); break;
						case 2: bd.setPixel32(i, scanline, upFilter()); break;
						case 3: bd.setPixel32(i, scanline, averageFilter()); break;
						case 4: bd.setPixel32(i, scanline, paethFilter()); break;
